...
protected:
	virtual ReportItem::PrintResult processPage(QPrinter *printer, ReportItemMetaPaint *out);
	/// return NULL if such a page does not exist.
	ReportItemMetaPaintFrame *getPage(int page_no);
public:
	void process(QPrinter *printer, ProcessorMode mode = AllPages);
...
public slots:
	...
	void processSinglePage(QPrinter *printer) {process(printer, SinglePage);}
public:
...
private:
	...
	QVariantMap m_data;
	// std::shared_ptr<QPrinter> m_currentPrinter = nullptr;
