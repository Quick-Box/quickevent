...
ReportItem::PrintResult ReportItemBand::printMetaPaint(QPrinter *printer, ReportItemMetaPaint *out, const ReportItem::Rect &bounding_rect)
{
	qfLogFuncFrame() << this;
	...
	if(m->rowCount() > 0) {
		res = Super::printMetaPaint(printer, out, bounding_rect);
	}
