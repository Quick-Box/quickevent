...
ReportItem::PrintResult ReportItemFrame::printMetaPaintChildren(QPrinter *printer, ReportItemMetaPaint *out, const ReportItem::Rect &bounding_rect)
{
	...
	if(layout() == LayoutStacked) {
		...
		for(m_indexToPrint=0; m_indexToPrint<itemsToPrintCount(); m_indexToPrint++) {
			...
			PrintResult ch_res = it->printMetaPaint(printer, out, children_paint_area_rect);
		...
		for(int i=0; i<itemsToPrintCount(); i++) {
			...
			if(sz.unit == Rect::UnitMM) {
				...
				PrintResult ch_res = it->printMetaPaint(printer, out, ch_bbr);
		...
		for(; m_indexToPrint<itemsToPrintCount(); m_indexToPrint++) {
			...
			int prev_children_cnt = out->childrenCount();
			PrintResult ch_res = child_item_to_print->printMetaPaint(printer, out, children_paint_area_rect);
			if(ch_res.isPrintFinished()) {
...
ReportItem::PrintResult ReportItemFrame::printMetaPaint(QPrinter *printer, ReportItemMetaPaint *out, const ReportItem::Rect &bounding_rect)
{
	qfLogFuncFrame() << this;
	qfDebug() << "\tbounding_rect:" << bounding_rect.toString();
	...
	if(column_sizes.isEmpty()) {
		...
		column_sizes = qf::gui::graphics::makeLayoutSizes(sl, ly_size);
	}
	...
	metapaint_frame->setLayout((qf::gui::graphics::Layout)layout());
	...
	for(int current_column_index=0; current_column_index<column_sizes.count(); current_column_index++) {
		...
		res = printMetaPaintChildren(printer, metapaint_frame, column_br);
