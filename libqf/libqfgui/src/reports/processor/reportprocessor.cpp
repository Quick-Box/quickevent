...
void ReportProcessor::process(QPrinter* printer, ReportProcessor::ProcessorMode mode)
{
	...
	while(m_singlePageProcessResult.isPrintAgain()) {
		{
			...
			m_singlePageProcessResult = processPage(printer, &mpit);
...
ReportItem::PrintResult ReportProcessor::processPage(QPrinter *printer, ReportItemMetaPaint *out)
{
	qfLogFuncFrame();
	...
	if(documentInstanceRoot()) {
		...
		res = documentInstanceRoot()->printMetaPaint(printer, out, ReportItem::Rect());
