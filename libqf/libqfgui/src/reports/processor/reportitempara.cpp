...
ReportItem::PrintResult ReportItemPara::printMetaPaint(QPrinter *printer, ReportItemMetaPaint *out, const Rect &bounding_rect)
{
	qfLogFuncFrame();
	return Super::printMetaPaint(printer, out, bounding_rect);
}

ReportItem::PrintResult ReportItemPara::printMetaPaintChildren(QPrinter *printer, ReportItemMetaPaint *out, const Rect &bounding_rect)
{
	...
	if(text.isEmpty() && omit_empty_text) {
		...
		auto font_metrics = QFontMetricsF(style.font(), printer);
		...
		device_bounding_rect = gui::graphics::mm2device(bounding_rect, printer);
		...
		device_bounding_rect = gui::graphics::device2mm(device_bounding_rect, printer);
